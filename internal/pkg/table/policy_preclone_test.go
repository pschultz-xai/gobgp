...
func TestProvablyRejectsPreCloneDifferential(t *testing.T) {
	...
	// just misses. TestPrefixSetMatchRtcPrefix covers deterministic RTC
	// set matching (it has no unmasked-bits entry; that shape is only
	// exercised here).
	prefixesRtc := []string{
		...
		for k := 0; k < 8; k++ {
			...
			case 2, 3: // RTC (Bendrr R-258)
				...
				case 3: // RT whose trie key is unsupported: nlriToPrefix
					// yields an invalid prefix and PrefixCondition returns
					// false under BOTH options — the invalid-key early
					// return precedes the INVERT flip (measured; see the
					// reconstructed-option pin below)
